...
import { useState, useEffect } from 'react'; // Assurez-vous d'importer useState
...
const HomePage = () => {
  const [showText, setShowText] = useState(true); // Ajoutez cet état
  useEffect(() => {
    // Place your JavaScript code here
    const canvas = document.getElementById('nightSky');
    const ctx = canvas.getContext('2d');
    canvas.addEventListener('click', function() {
      setShowText(false); // Cachez le texte lorsque le canvas est cliqué
    });
...
spawnRandomShootingStar();
  ...
  return (
    <div id="canvasContainer" className={styles.canvasContainer}>
      <canvas id="nightSky" className={`${styles.nightSky} ${styles.myCanvas}`}></canvas>
      {showText && <h1 className={`${styles.bigBoldText} ${styles.blinkingText}`} onClick={() => setShowText(false)}>Double Click</h1>}
      <TextDesign /> 
    </div>
  );
};
